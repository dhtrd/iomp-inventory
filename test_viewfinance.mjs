...
const CN=[{code:'99916382',qty:12},{code:'A1',qty:3}];
...
{ const page=await ctx.newPage(); await load(page,{profile:VW,users:[OWNER,VW],sessions:[S1]});
  ...
  ok('F1 رؤوس الجدول ٧ أعمدة كمّية بلا أي عمود مالي', h.length===7&&!h.includes('التكلفة')&&!h.includes('القيمة الدفترية')&&!h.includes('قيمة الفرق'), JSON.stringify(h));
  ...
  ok('F1 صفّ الإجماليات بلا خلايا قيم مالية (٥ خلايا) ولا يتضمن 56', tf.n===5&&!tf.txt.includes('56'), JSON.stringify(tf.n)+' '+tf.txt.slice(0,80));
  await page.close(); }
...
{ const page=await ctx.newPage(); await load(page,{profile:VW,users:[OWNER,VW],config:{users:{u_vw:{'report.finance':true}}},sessions:[S1]});
  ...
  const h=await ths(page);
  ok('F5 الأعمدة المالية عادت (١١ عمودًا)', h.length===11&&h.includes('التكلفة'), JSON.stringify(h));
